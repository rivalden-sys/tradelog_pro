    ...
    return (
      <div style={{ marginBottom: 20 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
          ...
          <span style={{ fontSize: 16, fontWeight: 800, color: color || textColor, letterSpacing: '-0.02em' }}>
        ...
        <input
          ...
          style={{ width: '100%', accentColor: color || BLUE, cursor: 'pointer' }}
        />
      ...
      <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', height: 'auto', display: 'block' }}>
        ...
        {yLabels.map(({ v, y }) => (
          <g key={v}>
            ...
            <text x={padding.left - 6} y={y + 4} textAnchor="end" fontSize={10} fill={axisColor}>
              ${Math.round(v)}
            </text>
          </g>
        ))}
        ...
        {/* Start balance line */}
        <line
          x1={padding.left} y1={yScale(startBalance)}
          x2={W - padding.right} y2={yScale(startBalance)}
          stroke={dark ? 'rgba(255,255,255,0.2)' : 'rgba(0,0,0,0.15)'}
          strokeWidth={1} strokeDasharray="4 4"
        />

        {/* Median path */}
        <path d={toSvgPath(medianPath)} fill="none" stroke={medianColor} strokeWidth={2.5} strokeLinecap="round" />

        {/* End dot on median */}
        <circle
          cx={xScale(months)} cy={yScale(medianPath[months])}
          r={5} fill={medianColor} stroke={dark ? '#0a0a0b' : '#fff'} strokeWidth={2}
        />
      </svg>
    )
  }

  const expValue = result ? Math.round(
        ...
        <div style={{ maxWidth: 1100, margin: '0 auto', padding: '24px 16px' }}>
          ...
          <div style={{ marginBottom: 28 }}>
            <h1 style={{ fontSize: 22, fontWeight: 800, color: textColor, margin: 0, letterSpacing: '-0.04em' }}>
              📈 {isUk ? 'Performance Simulator' : 'Performance Simulator'}
            </h1>
            <div style={{ fontSize: 13, color: subColor, marginTop: 4 }}>
              {isUk
                ? 'Monte Carlo симуляція — що буде якщо торгувати так само ще кілька місяців'
                : 'Monte Carlo simulation — what happens if you keep trading the same way'}
            </div>
          </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <div style={glassCard()}>
                ...
                <div style={{ position: 'relative' }}>
                  <Slider
                    label={isUk ? 'Win Rate' : 'Win Rate'}
                    value={winRate} min={10} max={90} unit="%"
                    color={winRate >= 50 ? GREEN : RED}
                    onChange={setWinRate}
                  />
                  <Slider
                    label={isUk ? 'Середній прибуток' : 'Average Win'}
                    value={avgWin} min={10} max={1000} step={10} unit="$"
                    color={GREEN}
                    onChange={setAvgWin}
                  />
                  <Slider
                    label={isUk ? 'Середній збиток' : 'Average Loss'}
                    value={avgLoss} min={10} max={1000} step={10} unit="$"
                    color={RED}
                    onChange={setAvgLoss}
                  />
                  <Slider
                    label={isUk ? 'Угод на місяць' : 'Trades per Month'}
                    value={tradesPerMonth} min={1} max={100}
                    color={BLUE}
              ...
              <div style={glassCard()}>
                ...
                <div style={{ position: 'relative' }}>
                  <Slider
                    label={isUk ? 'Стартовий баланс' : 'Starting Balance'}
                    value={startBalance} min={100} max={100000} step={100} unit="$"
                    color={PURPLE}
                    onChange={setStartBalance}
                  />
                  <Slider
                    label={isUk ? 'Горизонт (місяців)' : 'Horizon (months)'}
                    value={months} min={1} max={24}
                    color={ORANGE}
                    onChange={setMonths}
                  />
                </div>
              ...
              {result && (
                <div style={glassCard()}>
                  ...
                  <div style={{ fontSize: 14, fontWeight: 700, color: textColor, marginBottom: 16, position: 'relative' }}>
                    {isUk ? '📊 Результати після ' : '📊 Results after '}{months}{isUk ? ' місяців' : ' months'}
                  </div>
