    ...
    try {
      ...
      if (!json.success) {
        if (json.code === 'FREE_LIMIT_REACHED') {
          setImportError('Ліміт Free плану. Upgrade to Pro для необмеженого імпорту.')
        } else {
          setImportError(json.error || 'Import error')
        }
        return
      }
        ...
        >
          ...
          >
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
              <div>
                <div style={{ fontSize: 18, fontWeight: 800, color: textColor }}>📥 Імпорт угод з CSV</div>
                <div style={{ fontSize: 13, color: subColor, marginTop: 4 }}>
                  {importStep === 'upload'  && 'Завантажте файл з вашої біржі'}
                  {importStep === 'mapping' && 'Формат не визначено — вкажіть колонки вручну'}
                  {importStep === 'preview' && `Знайдено ${importTotal} угод — перевірте перед імпортом`}
                  {importStep === 'done'    && 'Імпорт завершено успішно!'}
                </div>
              </div>
            ...
            {importStep === 'upload' && (
              <div>
                ...
                <div style={{ background: dark ? 'rgba(10,132,255,0.08)' : LIGHT.blueBg, border: `1px solid ${BLUE}22`, borderRadius: 14, padding: '14px 16px', marginBottom: 20 }}>
                  <div style={{ fontSize: 13, fontWeight: 700, color: BLUE, marginBottom: 10 }}>Підтримувані біржі:</div>
                  ...
                  <div style={{ fontSize: 11, color: subColor, marginTop: 8 }}>* Bitget потребує ручного маппінгу колонок</div>
                </div>

                {/* Drop zone */}
                <label style={{
                  display: 'block', border: `2px dashed ${importFile ? GREEN : borderColor}`,
                  borderRadius: 16, padding: '32px 20px', textAlign: 'center', cursor: 'pointer',
                  background: importFile ? (dark ? `${DARK.green}0a` : LIGHT.greenBg) : 'transparent',
                  transition: 'all 0.2s',
                }}>
                  ...
                  <div style={{ fontSize: 15, fontWeight: 700, color: textColor, marginBottom: 6 }}>
                    {importFile ? importFile.name : 'Клікніть або перетягніть CSV файл'}
                  </div>
                  <div style={{ fontSize: 12, color: subColor }}>Підтримується .csv та .xlsx</div>
                </label>

                {importError && (
                  <div style={{ marginTop: 12, padding: '10px 14px', borderRadius: 10, background: `${RED}12`, color: RED, fontSize: 13 }}>{importError}</div>
                )}

                <button
                  disabled={!importFile || importLoading}
                  onClick={() => analyzeFile(false)}
                  style={{
                    ...
                    boxShadow: importFile && !dark ? '0 4px 14px rgba(0,0,0,0.25), inset 0 1px 0 rgba(255,255,255,0.2)' : 'none',
                  }}
                >{importLoading ? '⏳ Аналізуємо...' : 'Аналізувати файл →'}</button>
              </div>
            )}
            ...
            {importStep === 'mapping' && (
              <div>
                <div style={{ padding: '12px 16px', borderRadius: 12, background: `${ORANGE}12`, color: ORANGE, fontSize: 13, marginBottom: 20 }}>
                  ⚠️ Біржа не визначена автоматично. Вкажіть відповідність колонок.
                </div>

                {[
                  { key: 'pair',        label: 'Торгова пара',              required: true  },
                  { key: 'side',        label: 'Напрямок (Buy/Sell/Long/Short)', required: true  },
                  { key: 'pnl',         label: 'P&L / Прибуток (USDT)',     required: true  },
                  { key: 'date',        label: 'Дата угоди',                required: true  },
                  { key: 'entry_price', label: 'Ціна входу',                required: false },
                  { key: 'exit_price',  label: 'Ціна виходу',               required: false },
                ].map(field => (
                  <div key={field.key} style={{ marginBottom: 14 }}>
                    <label style={{ fontSize: 12, color: subColor, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.04em', display: 'block', marginBottom: 6 }}>
                      {field.label} {field.required ? <span style={{ color: RED }}>*</span> : <span style={{ color: subColor, fontSize: 10 }}>(опційно)</span>}
                    </label>
                    <select
                ...
                <div style={{ marginBottom: 20 }}>
                  <label style={{ fontSize: 12, color: subColor, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.04em', display: 'block', marginBottom: 8 }}>Тип угод</label>
                ...
                {importError && (
                  <div style={{ padding: '10px 14px', borderRadius: 10, background: `${RED}12`, color: RED, fontSize: 13, marginBottom: 12 }}>{importError}</div>
                )}

                <div style={{ display: 'flex', gap: 10 }}>
                  <button onClick={() => setImportStep('upload')} style={{ flex: 1, padding: '12px', borderRadius: 12, border: `1px solid ${borderColor}`, background: 'transparent', color: subColor, fontSize: 14, fontWeight: 600, cursor: 'pointer', fontFamily: FONT }}>← Назад</button>
                  <button
                    disabled={!mapping.pair || !mapping.side || !mapping.pnl || !mapping.date || importLoading}
                    onClick={() => analyzeFile(true)}
                    style={{ flex: 2, padding: '12px', borderRadius: 12, border: 'none', background: dark ? DARK.green : 'linear-gradient(180deg, #1f9e3f 0%, #166b2b 100%)', color: '#fff', fontSize: 14, fontWeight: 700, cursor: 'pointer', fontFamily: FONT }}
                  >{importLoading ? '⏳ Перевіряємо...' : 'Переглянути →'}</button>
                </div>
              </div>
            )}
              ...
              <div>
                {importFormat !== 'unknown' && (
                  <div style={{ padding: '10px 16px', borderRadius: 12, background: `${GREEN}12`, color: GREEN, fontSize: 13, fontWeight: 600, marginBottom: 16 }}>
                    ✓ Формат: {FORMAT_LABELS[importFormat]}
                  </div>
                )}

                <div style={{ fontSize: 13, color: subColor, marginBottom: 12 }}>
                  Перші {importPreview.length} з {importTotal} угод:
                </div>
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
                    <thead>
                      <tr style={{ borderBottom: `1px solid ${borderColor}`, background: dark ? 'rgba(255,255,255,0.03)' : 'rgba(0,0,0,0.02)' }}>
                        {['Дата', 'Пара', 'Напрямок', 'P&L $', 'Результат', 'Тип'].map(h => (
                ...
                <div style={{ display: 'flex', gap: 10 }}>
                  ...
                  <button
                    ...
                    onClick={doImport}
                    style={{ flex: 2, padding: '12px', borderRadius: 12, border: 'none', background: dark ? DARK.green : 'linear-gradient(180deg, #1f9e3f 0%, #166b2b 100%)', color: '#fff', fontSize: 14, fontWeight: 700, cursor: 'pointer', fontFamily: FONT, boxShadow: !dark ? '0 4px 14px rgba(0,0,0,0.25)' : 'none' }}
                  >{importLoading ? '⏳ Імпортуємо...' : `✓ Імпортувати ${importTotal} угод`}</button>
                </div>
              </div>
            )}
            ...
            {importStep === 'done' && importResult && (
              <div style={{ textAlign: 'center', padding: '20px 0' }}>
                ...
                <div style={{ fontSize: 22, fontWeight: 800, color: textColor, marginBottom: 8 }}>Імпорт завершено!</div>
                <div style={{ fontSize: 15, color: subColor, marginBottom: 28 }}>
                  Додано <span style={{ color: GREEN, fontWeight: 800 }}>{importResult.imported} угод</span>
                  {importResult.skipped > 0 && <span style={{ color: subColor }}> · пропущено {importResult.skipped}</span>}
                </div>
                <button
                  onClick={resetImport}
                  style={{ padding: '13px 32px', borderRadius: 12, border: 'none', background: dark ? DARK.green : 'linear-gradient(180deg, #1f9e3f 0%, #166b2b 100%)', color: '#fff', fontSize: 15, fontWeight: 700, cursor: 'pointer', fontFamily: FONT }}
                >
                  Переглянути журнал
                </button>
              </div>
