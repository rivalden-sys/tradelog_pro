...
export default function AnalyticsPage() {
  ...
  trades.forEach(tr => {
    if (!setupMap[tr.setup]) setupMap[tr.setup] = { wins: 0, total: 0, pnl: 0 }
    setupMap[tr.setup].total++
    setupMap[tr.setup].pnl += tr.profit_usd || 0
    if (tr.result === 'Тейк') setupMap[tr.setup].wins++
  })
  const bySetup = Object.entries(setupMap)
    .map(([setup, v]) => ({
      setup: setup.split('+')[0].trim(),
  ...
  trades.forEach(tr => {
    if (!pairMap[tr.pair]) pairMap[tr.pair] = { wins: 0, total: 0, pnl: 0 }
    pairMap[tr.pair].total++
    pairMap[tr.pair].pnl += tr.profit_usd || 0
    if (tr.result === 'Тейк') pairMap[tr.pair].wins++
  })
  ...
  trades.forEach(tr => {
    if (tr.self_grade) gradeMap[tr.self_grade] = (gradeMap[tr.self_grade] || 0) + 1
  })
  ...
  trades.forEach(tr => {
    if (!tr.rr) return
    const bucket = `${Math.floor(tr.rr * 2) / 2}`
    rrBuckets[bucket] = (rrBuckets[bucket] || 0) + 1
  })
  const rrDist = Object.entries(rrBuckets)
    .map(([rr, count]) => ({ rr, count }))
    .sort((a, b) => parseFloat(a.rr) - parseFloat(b.rr))

  const monthMap: Record<string, number> = {}
  trades.forEach(tr => {
    const month = tr.date.slice(0, 7)
    monthMap[month] = (monthMap[month] || 0) + (tr.profit_usd || 0)
  })
  const byMonth = Object.entries(monthMap)
    .map(([month, pnl]) => ({ month: month.slice(5), pnl: Math.round(pnl * 100) / 100 }))
    .sort((a, b) => a.month.localeCompare(b.month))

  const disciplineTrades = trades.filter(tr => tr.self_grade === 'C' || tr.self_grade === 'D')
  const disciplineScore = trades.length
    ? Math.round(((trades.length - disciplineTrades.length) / trades.length) * 100)
    : 0

  const longs  = trades.filter(tr => tr.direction === 'Long')
  const shorts = trades.filter(tr => tr.direction === 'Short')
  const longWR  = longs.length  ? Math.round((longs.filter(tr  => tr.result === 'Тейк').length / longs.length)  * 100) : 0
  const shortWR = shorts.length ? Math.round((shorts.filter(tr => tr.result === 'Тейк').length / shorts.length) * 100) : 0
    ...
    <div style={{ minHeight: '100vh', background: t.bg, fontFamily: FONT, transition: 'background 0.3s' }}>
      ...
      <div style={{ padding: '32px 40px' }}>
        ...
        <div style={{ marginBottom: 28 }}>
          <div style={{ fontSize: 26, fontWeight: 800, color: t.text, letterSpacing: '-0.04em' }}>Аналитика</div>
          <div style={{ fontSize: 13, color: t.sub, marginTop: 2 }}>{trades.length} сделок всего</div>
        </div>

        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 14, marginBottom: 24 }}>
          {[
            { label: 'Дисциплина',     value: `${disciplineScore}%`,              color: disciplineScore >= 70 ? GREEN : RED },
            { label: 'Long Win Rate',  value: `${longWR}%`,                       color: longWR  >= 50 ? GREEN : RED },
            { label: 'Short Win Rate', value: `${shortWR}%`,                      color: shortWR >= 50 ? GREEN : RED },
            { label: 'Лонгов / Шортов', value: `${longs.length} / ${shorts.length}`, color: t.text },
          ].map(sc => (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
          <div style={card(t)}>
            <div style={{ fontSize: 15, fontWeight: 700, color: t.text, marginBottom: 18, letterSpacing: '-0.02em' }}>Win Rate по сетапам</div>
            {bySetup.length === 0 ? (
              <div style={{ color: t.sub, fontSize: 13, textAlign: 'center', padding: '32px 0' }}>Нет данных</div>
            ) : (
          ...
          <div style={card(t)}>
            <div style={{ fontSize: 15, fontWeight: 700, color: t.text, marginBottom: 18, letterSpacing: '-0.02em' }}>P&L по месяцам</div>
            {byMonth.length === 0 ? (
              <div style={{ color: t.sub, fontSize: 13, textAlign: 'center', padding: '32px 0' }}>Нет данных</div>
            ) : (
              <ResponsiveContainer width="100%" height={200}>
                <BarChart data={byMonth} barSize={28}>
                  ...
                  <Tooltip
                    contentStyle={{ background: t.surface, border: `1px solid ${t.border}`, borderRadius: 10, fontFamily: FONT }}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
          <div style={card(t)}>
            <div style={{ fontSize: 15, fontWeight: 700, color: t.text, marginBottom: 18, letterSpacing: '-0.02em' }}>Статистика по парам</div>
            {byPair.length === 0 ? (
              <div style={{ color: t.sub, fontSize: 13, textAlign: 'center', padding: '32px 0' }}>Нет данных</div>
            ) : (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div style={card(t)}>
              <div style={{ fontSize: 15, fontWeight: 700, color: t.text, marginBottom: 16, letterSpacing: '-0.02em' }}>Распределение оценок</div>
                ...
                {byGrade.map(g => {
                  ...
                  return (
                    <div key={g.grade} style={{
                      ...
                      background: (colors[g.grade] || t.sub) + '18',
                    }}>
            ...
            <div style={card(t)}>
              <div style={{ fontSize: 15, fontWeight: 700, color: t.text, marginBottom: 16, letterSpacing: '-0.02em' }}>Распределение RR</div>
              {rrDist.length === 0 ? (
                <div style={{ color: t.sub, fontSize: 13, textAlign: 'center', padding: '16px 0' }}>Нет данных</div>
              ) : (
