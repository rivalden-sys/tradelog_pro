...
export default function PlaybookPage() {
  ...
  const load = async () => {
    ...
    // Завантажуємо статистику для кожного playbook
    const statsMap: Record<string, PlaybookStats> = {}

    for (const pb of pbData) {
      // Всі rule checks для цього playbook
      ...
      trades.forEach(t => { tradeResults[t.id] = t.result })
      ...
      <div style={{ position: 'relative', zIndex: 1 }}>
        ...
        <div style={{ maxWidth: 720, margin: '0 auto', padding: '24px 16px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24, flexWrap: 'wrap', gap: 12 }}>
            <div>
              <h1 style={{ fontSize: 22, fontWeight: 800, color: textColor, margin: 0, letterSpacing: '-0.04em' }}>📋 Playbook</h1>
              <div style={{ fontSize: 13, color: subColor, marginTop: 4 }}>Правила твоїх сетапів — дотримання vs результат</div>
            </div>
            {!showForm && (
          ...
          {showForm && (
            <div style={{ ...glassCard(editingId ? BLUE : GREEN), marginBottom: 20 }}>
              ...
              <div style={{ marginBottom: 20, position: 'relative' }}>
                ...
                <input type="text" placeholder="CHoCH + BOS + FVG" value={setupName} onChange={e => setSetupName(e.target.value)} style={inputStyle} autoFocus />
              </div>
              <div style={{ marginBottom: 20, position: 'relative' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  {rules.map((rule, i) => (
                    <div key={rule.id} style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                      ...
                      <input type="text"
                        placeholder={i === 0 ? 'напр. CHoCH на H1 підтверджено' : i === 1 ? 'напр. BOS в напрямку угоди' : i === 2 ? 'напр. FVG або OB в зоні входу' : 'Додати правило...'}
              ...
              <div style={{ display: 'flex', gap: 10, position: 'relative' }}>
                ...
                }}>{saving ? 'Збереження...' : editingId ? 'Зберегти зміни' : 'Створити playbook'}</button>
                <button onClick={resetForm} style={{ padding: '11px 20px', borderRadius: 12, border: `1px solid ${borderColor}`, background: 'transparent', color: subColor, fontSize: 14, fontWeight: 600, cursor: 'pointer', fontFamily: FONT }}>Скасувати</button>
              </div>
            </div>
          )}
          ...
          {/* List */}
          {loading ? (
            <div style={{ textAlign: 'center', padding: '48px 0', color: subColor, fontSize: 14 }}>Завантаження...</div>
          ) : playbooks.length === 0 && !showForm ? (
            <div style={{ ...glassCard(), textAlign: 'center', padding: '48px 20px' }}>
              <div style={{ fontSize: 40, marginBottom: 16 }}>📋</div>
              <div style={{ fontSize: 16, fontWeight: 700, color: textColor, marginBottom: 8 }}>Playbook ще порожній</div>
                  ...
                  <div key={pb.id} style={glassCard()}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, marginBottom: 16, position: 'relative' }}>
                      <div>
                        ...
                        <div style={{ fontSize: 12, color: subColor, marginTop: 2 }}>
                          ...
                          {hasStats && ` · ${s.totalTrades} угод`}
                        </div>
                      </div>
                      <div style={{ display: 'flex', gap: 8 }}>
                        <button onClick={() => startEdit(pb)} style={{ padding: '6px 14px', borderRadius: 8, border: `1px solid ${borderColor}`, background: 'transparent', color: subColor, fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: FONT }}>✏️ Редагувати</button>
                      ...
                      <div style={{ marginBottom: 16, position: 'relative' }}>
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 10 }}>
                          ...
                          <div style={{
                            ...
                            background: dark ? 'rgba(48,209,88,0.08)' : 'rgba(48,209,88,0.07)',
                            border: `1px solid ${GREEN}33`,
                          }}>
                            <div style={{ fontSize: 11, color: GREEN, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.04em', marginBottom: 6 }}>✓ Дотримувався</div>
                            <div style={{ display: 'flex', alignItems: 'baseline', gap: 6 }}>
                              <div style={{ fontSize: 26, fontWeight: 900, color: GREEN, letterSpacing: '-0.03em' }}>
                                {s.followedAllWR}%
                              </div>
                              <div style={{ fontSize: 12, color: subColor }}>win rate</div>
                            </div>
                            <div style={{ fontSize: 12, color: subColor, marginTop: 2 }}>{s.followedAll} угод</div>
                            {/* Progress bar */}
                          ...
                          <div style={{
                            padding: '12px 14px', borderRadius: 12,
                            background: dark ? 'rgba(255,69,58,0.08)' : 'rgba(255,69,58,0.06)',
                            border: `1px solid ${RED}33`,
                          }}>
                            <div style={{ fontSize: 11, color: RED, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.04em', marginBottom: 6 }}>✕ Порушував</div>
                            <div style={{ display: 'flex', alignItems: 'baseline', gap: 6 }}>
                              <div style={{ fontSize: 26, fontWeight: 900, color: RED, letterSpacing: '-0.03em' }}>
                                {s.violatedAnyWR}%
                              </div>
                              <div style={{ fontSize: 12, color: subColor }}>win rate</div>
                            </div>
                            <div style={{ fontSize: 12, color: subColor, marginTop: 2 }}>{s.violatedAny} угод</div>
                            <div style={{ height: 4, borderRadius: 2, background: 'rgba(128,128,128,0.15)', overflow: 'hidden', marginTop: 8 }}>
                        ...
                        {s.followedAll > 0 && s.violatedAny > 0 && (
                          <div style={{
                            ...
                            {s.followedAllWR > s.violatedAnyWR
                              ? `📈 Дотримання правил дає +${s.followedAllWR - s.violatedAnyWR}% до win rate`
                              : `⚠️ Порушення правил не впливає критично — перевір систему`
                            }
                          </div>
                        )}
                        {s.totalTrades > 0 && s.followedAll === 0 && (
                          <div style={{ padding: '10px 14px', borderRadius: 10, fontSize: 13, background: dark ? 'rgba(255,159,10,0.08)' : 'rgba(255,159,10,0.07)', border: `1px solid ${ORANGE}33`, color: ORANGE, fontWeight: 600 }}>
                            ⚠️ Ти ще жодного разу не дотримався всіх правил
                          </div>
                        )}
                      </div>
                    )}
                    ...
                    {!hasStats && (
                      <div style={{ marginBottom: 16, padding: '10px 14px', borderRadius: 10, background: dark ? 'rgba(255,255,255,0.03)' : 'rgba(0,0,0,0.03)', border: `1px solid ${borderColor}`, fontSize: 13, color: subColor, position: 'relative' }}>
                        Статистика з'явиться після перших угод з цим playbook
                      </div>
                    )}
